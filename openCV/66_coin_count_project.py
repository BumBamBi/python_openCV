...
# 동전 표면을 흐릿하게 하여 단색의 원처럼 보일 수 있도록, 피라미드평균시프트 적용
mean = cv2.pyrMeanShiftFiltering(img, 20, 50)
cv2.imshow('mean', mean)

# otsu로 이진화시킴 (그전에, GRAY화 + Gaussian으로 잡음제거)
...
# 거리 변환 결과값의 절반 이상을 차지한 영역은 확실한 동전으로 인식
...
# 불확실한 영역 설정 : 확실한 배경을 반전해서 확실한 부분들을 빼기
